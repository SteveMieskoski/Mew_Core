...
test('sign transaction (via engine)', function (t) {
  ...
  let otherWalletProvider = new HardwareWalletProvider(fromFile)
  let hardwareWalletProvider = new HardwareWalletProvider(new LedgerWallet())
  // sign all tx's
  var providerA = injectMetrics(hardwareWalletProvider)
  var providerReplace = injectMetrics(otherWalletProvider)

  var engine = new MewEngine()
  engine.addProvider(providerA)

  engine.setNewWalletProvider(providerReplace);
  var txPayload = {
...
console.log(response); // todo remove dev item
    // intial tx request
    t.equal(providerA.getWitnessed('eth_signTransaction').length, 0, 'providerA did see "signTransaction"')
    t.equal(providerA.getHandled('eth_signTransaction').length, 0, 'providerA did handle "signTransaction"')
    t.equal(providerReplace.getWitnessed('eth_signTransaction').length, 1, 'providerReplace did see "signTransaction"')
    t.equal(providerReplace.getHandled('eth_signTransaction').length, 1, 'providerReplace did handle "signTransaction"')
